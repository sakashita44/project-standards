...
- `general/mise.local.toml.example` への `disable_tools` の設定例。プロジェクトの `mise.toml` が宣言したツールのうちホストの mise で解決できないものがあると、mise はコマンド実行のたびに警告を出し `mise install` も失敗する。`mise.local.toml` で無視すれば、他の利用者の設定を変えずに済む
